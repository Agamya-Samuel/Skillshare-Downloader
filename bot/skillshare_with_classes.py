...
class Skillshare(object):
    ...
    def __init__(self, url):
        ...
        self.file_name = "None"
        self.size = None
        self.anonfile_url = None
        self.pixeldrain_url = None
    ...
    def download_course_by_class_id(self):
        ...
        if Skillshare.is_unicode_string(teacher_name):
    ...
    def download_course_by_url(self):
        m = re.match(r'https://www.skillshare.com/classes/.*?/(\d+)', self.url)
        if m is None:
            m = re.match(r'https://www.skillshare.com/[a-zA-Z]+/classes/.*?/(\d+)', self.url)

        if m is None:
            raise Exception('Failed to parse class ID from URL')

        self.course_id = m.group(1)
        self.download_course_by_class_id()